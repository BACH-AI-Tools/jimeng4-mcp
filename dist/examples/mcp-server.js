...
const dotenvSilent = __importStar(require("../src/dotenv-silent"));
...
try {
    ...
    if ((!envResult || !envResult.parsed || Object.keys(envResult.parsed).length === 0) &&
        ...
        const homeEnvPath = path_1.default.join(process.env.HOME || process.env.USERPROFILE || "", ".jimeng-ai-mcp", ".env");
...
server.tool("jimeng-v40-generate", "当用户需要生成图片时使用的工具 - 即梦4.0图片生成工具，支持文生图、图像编辑及多图组合生成（最多10张输入图，最多15张输出图），支持4K超高清输出。此工具会等待图片生成完成后返回结果。", {
    prompt: zod_1.z.string().describe("生成图像的提示词，中英文均可，最长800字符"),
    image_urls: zod_1.z.array(zod_1.z.string()).optional().describe("输入图片URL数组，最多10张，支持JPEG/PNG格式，最大15MB"),
    width: zod_1.z.number().optional().describe("图像宽度，需与height同时传入才生效"),
    height: zod_1.z.number().optional().describe("图像高度，需与width同时传入才生效"),
    size: zod_1.z.number().optional().describe("生成图片的尺寸，可以是边长（如2048表示2048×2048）或面积（如4194304）。支持常见尺寸：1024/2048/4096表示正方形，或直接传面积值"),
    scale: zod_1.z.number().optional().describe("文本描述影响程度，范围[0,1]，默认0.5，越大文本影响越大"),
    force_single: zod_1.z.boolean().optional().describe("是否强制生成单图，默认false"),
    seed: zod_1.z.number().optional().describe("随机种子，默认-1（随机）"),
    min_ratio: zod_1.z.number().optional().describe("生图结果的最小宽高比，默认1/3"),
    max_ratio: zod_1.z.number().optional().describe("生图结果的最大宽高比，默认3")
}, async (args, _extra) => {
    var _a;
    if (!JIMENG_ACCESS_KEY || !JIMENG_SECRET_KEY) {
        return {
    ...
    try {
        const client = new src_1.JimengClient({ debug: false });
        // 智能处理size参数
        ...
        // 同步生成图片（内部轮询直到完成）
        log("即梦4.0图片生成中，请稍候...");
        const result = await client.generateJimengV40Image(processedArgs);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
            ...
            content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        data: {
                            ...
                            image_count: ((_a = result.image_urls) === null || _a === void 0 ? void 0 : _a.length) || 0,
                            task_id: result.task_id
                        }
                    }, null, 2)
                }]
...
// 注册即梦图生图3.0工具
server.tool("jimeng-i2i-v30", "即梦图生图3.0智能参考工具，支持基于文本指令进行图像编辑（如添加/删除实体、修改风格/色彩/动作/背景等）。此工具会等待图片编辑完成后返回结果。", {
    image_url: zod_1.z.string().describe("输入图片URL，支持JPEG/PNG格式，最大4.7MB"),
    prompt: zod_1.z.string().describe("编辑图像的提示词，建议<=120字符，最长800字符"),
    width: zod_1.z.number().optional().describe("生成图像宽度，范围[512,2016]，需与height同时传入"),
    height: zod_1.z.number().optional().describe("生成图像高度，范围[512,2016]，需与width同时传入"),
    scale: zod_1.z.number().optional().describe("文本描述影响程度，范围[0,1]，默认0.5"),
    seed: zod_1.z.number().optional().describe("随机种子，默认-1（随机）")
}, async (args, _extra) => {
    var _a;
    if (!JIMENG_ACCESS_KEY || !JIMENG_SECRET_KEY) {
...
// 注册即梦文生图3.1工具
server.tool("jimeng-t2i-v31", "即梦文生图3.1工具，画面效果呈现升级版，在画面美感、风格精准多样及细节丰富度方面显著提升，支持1K-2K高清输出。此工具会等待图片生成完成后返回结果。", {
    prompt: zod_1.z.string().describe("生成图像的提示词，中英文均可，建议<=120字符，最长800字符"),
    use_pre_llm: zod_1.z.boolean().optional().describe("是否开启文本扩写，prompt较短建议开启，较长建议关闭，默认true"),
    width: zod_1.z.number().optional().describe("生成图像宽度，需与height同时传入"),
    height: zod_1.z.number().optional().describe("生成图像高度，需与width同时传入"),
    seed: zod_1.z.number().optional().describe("随机种子，默认-1（随机）")
}, async (args, _extra) => {
    var _a;
    if (!JIMENG_ACCESS_KEY || !JIMENG_SECRET_KEY) {
    ...
    try {
        ...
        // 同步生成图片（内部轮询直到完成）
        log("即梦文生图3.1生成中，请稍候...");
        const result = await client.generateJimengT2IV31Image(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
...
// 注册即梦文生图3.0工具
server.tool("jimeng-t2i-v30", "即梦文生图3.0工具，在文字响应准确度、图文排版、层次美感和语义理解方面显著提升，支持各类艺术字体和不同字重。此工具会等待图片生成完成后返回结果。", {
    prompt: zod_1.z.string().describe("生成图像的提示词，中英文均可，建议<=120字符，最长800字符"),
    use_pre_llm: zod_1.z.boolean().optional().describe("是否开启文本扩写，默认true"),
    width: zod_1.z.number().optional().describe("生成图像宽度，需与height同时传入"),
    height: zod_1.z.number().optional().describe("生成图像高度，需与width同时传入"),
    seed: zod_1.z.number().optional().describe("随机种子，默认-1（随机）")
}, async (args, _extra) => {
    ...
    try {
        ...
        // 同步生成图片（内部轮询直到完成）
        log("即梦文生图3.0生成中，请稍候...");
        const result = await client.generateJimengT2IV30Image(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
