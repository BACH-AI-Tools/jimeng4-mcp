...
class JimengClient {
     ...
     * 火山引擎V4签名算法
     */
    signV4Request(reqQuery, reqBody, region) {
        const t = new Date();
    ...
    async submitAsyncTask(params) {
        try {
            ...
            // 发送请求
            const response = await axios_1.default.post(requestUrl, params, {
                headers: headers,
                timeout: this.timeout,
                validateStatus: null
            });
     ...
     */
    async waitForImageTaskResult(options) {
        var _a, _b, _c, _d;
        const initialDelayMs = (_a = options.initialDelayMs) !== null && _a !== void 0 ? _a : 8000;
        const maxDelayMs = (_b = options.maxDelayMs) !== null && _b !== void 0 ? _b : 20000;
        const backoffFactor = (_c = options.backoffFactor) !== null && _c !== void 0 ? _c : 1.5;
        const maxWaitMs = (_d = options.maxWaitMs) !== null && _d !== void 0 ? _d : 6 * 60 * 1000;
     ...
     * 即梦4.0图片生成 - 提交任务
     */
    async submitJimengV40Task(params) {
        // 参数验证和清理（根据API文档）
        const cleanParams = {
     ...
     * 即梦4.0图片生成 - 同步方法（内部轮询）
     */
    async generateJimengV40Image(params) {
     ...
     * 即梦图生图3.0 - 提交任务
     */
    async submitJimengI2IV30Task(params) {
        const taskParams = {
     ...
     * 即梦图生图3.0 - 同步方法（内部轮询）
     */
    async generateJimengI2IV30Image(params) {
     ...
     * 即梦文生图3.1 - 提交任务
     */
    async submitJimengT2IV31Task(params) {
        const taskParams = {
     ...
     * 即梦文生图3.1 - 同步方法（内部轮询）
     */
    async generateJimengT2IV31Image(params) {
     ...
     * 即梦文生图3.0 - 提交任务
     */
    async submitJimengT2IV30Task(params) {
        const taskParams = {
     ...
     * 即梦文生图3.0 - 同步方法（内部轮询）
     */
    async generateJimengT2IV30Image(params) {
    ...
    async getVideoTaskResult(taskId, reqKey = "jimeng_vgfm_t2v_l20") {
        ...
        while (retryCount <= retries) {
            try {
                ...
                // 生成签名和请求头
                const { headers, requestUrl } = this.signV4Request(formattedQuery, formattedBody, reqKey.split('_')[2] // 提取region
                );
                // 开启调试信息
...
// 验证键辅助函数
function verifyKeys(keys, required) {
